 ...
 *
 * Removes password protection from PDF files entirely client-side.
 *
 * How it works:
 * 1. User uploads a password-protected PDF
 * 2. User enters the PDF password
 * 3. The qpdf WASM module decrypts the PDF using the provided password
 * 4. The decrypted (unlocked) PDF is offered for download
 *
 ...
 *
 * Pattern references:
 * - PDFResizer: dropzone pattern, file info display, formatSize helper
 * - pdf-unlocker (github.com/OrBin/pdf-unlocker): qpdf-wasm usage pattern
 *
 * @component
 * @returns {React.ReactElement} The PDF Unlock tool page
 */
...
// so subsequent unlocks don't re-download it.
...
const PDFUnlock = () => {
    ...
    // --- Result state ---
    const [unlockedBlob, setUnlockedBlob] = useState(null);  // The decrypted PDF as a Blob
    const [unlockedSize, setUnlockedSize] = useState(0);      // Decrypted file size in bytes
    ...
    const loadQpdfModule = async () => {
        ...
        if (qpdfModuleCache) {
            console.log('[PDFUnlock] Using cached WASM module');
            return qpdfModuleCache;
        }

        console.log('[PDFUnlock] Loading qpdf WASM module...');
        ...
        console.log('[PDFUnlock] WASM module loaded successfully');
        return qpdf;
    };
    ...
    const onDrop = useCallback((acceptedFiles) => {
        ...
        if (acceptedFiles.length === 0) {
            console.log('[PDFUnlock] No files dropped');
            return;
        }

        const file = acceptedFiles[0];
        console.log(`[PDFUnlock] File selected: ${file.name} (${formatSize(file.size)})`);

        // Validate file type — only accept PDFs
        if (file.type !== 'application/pdf' && !file.name.toLowerCase().endsWith('.pdf')) {
            setError('Please select a PDF file.');
            console.warn('[PDFUnlock] Rejected non-PDF file:', file.type);
            return;
        }
        ...
        if (file.size > maxSize) {
            ...
            console.warn('[PDFUnlock] Rejected oversized file:', formatSize(file.size));
            return;
        }
    ...
    /**
     * Main unlock function. Loads the WASM module (if needed), writes the PDF
     * to Emscripten's virtual filesystem, runs qpdf --decrypt, and reads the output.
     */
    const handleUnlock = async () => {
        ...
        console.log(`[PDFUnlock] Starting unlock for: ${fileName}`);
        setError('');
        ...
        try {
            ...
            console.log(`[PDFUnlock] File read: ${uint8Array.length} bytes`);

            // Step 3: Write the PDF to Emscripten's virtual filesystem
            setProgress('Preparing file...');
            qpdf.FS.writeFile('/input.pdf', uint8Array);

            // Step 4: Run qpdf decrypt command
            // qpdf --password=<password> --decrypt /input.pdf /output.pdf
            setProgress('Unlocking PDF...');
            console.log('[PDFUnlock] Running qpdf --decrypt...');
            try {
                qpdf.callMain([
                    `--password=${password}`,
                    '--decrypt',
                ...
                // callMain throws on failure (wrong password, corrupt file, etc.)
                console.error('[PDFUnlock] qpdf error:', qpdfError);
                // Clean up the input file from virtual filesystem
                try { qpdf.FS.unlink('/input.pdf'); } catch (e) { /* ignore cleanup errors */ }
                throw new Error('Incorrect password or the PDF file may be corrupted.');
            }

            // Step 5: Read the decrypted output from virtual filesystem
            setProgress('Preparing download...');
            const decryptedBytes = qpdf.FS.readFile('/output.pdf');
            console.log(`[PDFUnlock] Decrypted output: ${decryptedBytes.length} bytes`);

            // Step 6: Create a Blob for download
            const blob = new Blob([decryptedBytes], { type: 'application/pdf' });
            setUnlockedBlob(blob);
            setUnlockedSize(decryptedBytes.length);
            ...
            try {
                ...
                console.warn('[PDFUnlock] Cleanup warning:', e);
            }

            console.log('[PDFUnlock] Unlock successful!');
        } catch (err) {
            console.error('[PDFUnlock] Unlock failed:', err);
    ...
    /**
     * Triggers download of the unlocked PDF file.
     * Generates a filename like "original_unlocked.pdf".
     */
    const handleDownload = () => {
        ...
        link.href = url;
        // Generate filename: "document.pdf" → "document_unlocked.pdf"
        ...
        console.log(`[PDFUnlock] Download triggered: ${link.download}`);
    };
    ...
    const resetTool = () => {
        ...
        setUnlockedSize(0);
        setError('');
        console.log('[PDFUnlock] Tool reset');
    };

    /**
     * Handles Enter key press in the password input to trigger unlock.
     */
        ...
        <PageWrapper>
            <div className="max-w-4xl mx-auto">
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-8">
                    Remove password protection from PDF files. All processing happens in your browser — your files are never uploaded.
                </p>
                ...
                {!pdfFile ? (
                    ...
                    >
                        <input {...getInputProps()} />
                        {/* Upload icon — lock SVG to indicate password-protected files */}
                        <svg
                    ...
                    <div className="space-y-6">
                        ...
                        {/* ─── Section 2: Password Input + Unlock Button ──────── */}
                        {!unlockedBlob && (
                        ...
                        {unlockedBlob && (
                            <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-6 text-center">
                                ...
                                <p className="text-green-700 dark:text-green-400 text-sm mb-4">
                                    Output size: {formatSize(unlockedSize)}
                                </p>

                                {/* Download + Unlock another buttons */}
                ...
                <div className="bg-blue-50 dark:bg-gray-700 rounded-lg p-4 mt-8">
                    ...
                    <ol className="list-decimal list-inside text-gray-600 dark:text-gray-300 space-y-1 text-sm">
                        <li>Upload a password-protected PDF file (drag & drop or click to browse)</li>
                        <li>Enter the PDF password</li>
                        <li>Click "Unlock PDF" to remove the password protection</li>
                        <li>Download the unlocked PDF — it will open without requiring a password</li>
                    </ol>
